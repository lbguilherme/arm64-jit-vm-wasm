...
defineInstruction({
  ...
  asm({imm26}) {
    return `b\t${immToString((signExtend(imm26, 26) * 4) + 4)}`;
  },
  jit(ctx, {imm26}) {
    ctx.branch(ctx.pc + signExtend(imm26, 26) * 4);
  }
});
...
defineInstruction({
  ...
  asm({imm26}) {
    return `bl\t${immToString((signExtend(imm26, 26) * 4) + 4)}`;
  },
  jit(ctx, {imm26}) {
    const funcIndex = ctx.getFuncIndex(ctx.pc + signExtend(imm26, 26) * 4);
